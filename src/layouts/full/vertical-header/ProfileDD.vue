...
org.value = getUserInfo()
var orgname = JSON.parse(localStorage.getItem('user') || '{}')

function getUserInfo():String | null{
  return orgname
}
</script>
            ...
            <div class="custom-hover-primary text-left px-0 cursor-pointer" variant="text" v-bind="props">
                <div class="d-flex align-center">
                    ...
                    <div class="ml-md-4 d-md-block d-none">
                        <h6 class="text-h6 d-flex align-center text-black font-weight-semibold">{{ orgname.OrgInfo }}</h6>
                        <span class="text-subtitle-2 font-weight-medium text-grey100">{{ orgname.UserName }}</span>
                    </div>
                </div>
            </div>
        </template>
        <v-sheet rounded="lg" width="385" elevation="10" class="mt-5">
            <div class="px-8 pt-6">
                <div class="d-flex align-center justify-space-between">
                    <h6 class="text-h5 font-weight-semibold">Foydalanuvchi ma'lumoti</h6>
                ...
                <div class="d-flex align-center mt-5 pb-6">
                    ...
                    <div class="ml-5">
                        <h6 class="text-h5 mb-n1">{{ orgname.OrgInfo }}</h6>
                        <span class="text-subtitle-1 font-weight-regular text-grey100 font-weight-medium">{{ orgname.UserName }}</span>
                        <div class="d-flex align-center mt-1">
                            <MailIcon size="18" stroke-width="1.5" class="text-grey100" />
                            <span class="text-subtitle-1 text-grey100 font-weight-medium ml-2">{{ orgname.OrgINN }}</span>
                        </div>
                    </div>
                </div>
                <v-divider></v-divider>
            ...
            <div class=" pb-6 px-8 text-center">
                <v-btn color="primary" size="large" rounded="pill" block @click="authStore.logout()">Chiqish</v-btn>
            </div>
        </v-sheet>
    </v-menu>
</template>
